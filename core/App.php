...
class App {
	...
	protected $params = [];
	protected $host= "http://localhost/php-login-example/";

	public function __construct() {
		$url = $this->parseUrl();
		if(!isset($url[4]))
			header('Location: ' . $this->host. 'index.php/Home/vistaFrontend');
		if(isset($url[5]))
			header('Location: ' . $this->host . 'index.php/Home/vistaNoEncontrada');
		if(isset($url[3])) {
			if(file_exists('controllers/' . $url[3] . '.php')) { // Si existe archivo que pertenece al controlador enviado en la url
				$this->controller = $url[3]; // Setea la variable de instancia controller con ese valor, sino va por defecto al controlador Home
				unset($url[3]); // Deja en blanco la posicion que contiene al controlador en arreglo $url
			}
			else {
				header('Location: ' . $this->host. 'index.php/Home/vistaNoEncontrada');
			}
		...
		if(isset($url[4])) {  // Si existe el metodo en la url
			$param = explode("?", $url[4]);
